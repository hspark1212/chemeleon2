...
def maybe_noise_like_with_mask(x, mask=None):
    """Add noise to x, but only where mask is True."""
    return (
        th.randn_like(x) if mask is None else th.randn_like(x) * broadcast_mask(mask, x)
    )
...
def mean_flat(tensor, mask=None):
    ...
    if mask is not None:
        ...
        summed = (tensor * mask).sum(dim=reduced_dims)
        count = mask.sum(dim=reduced_dims).clamp(min=1e-6)  # Avoid division by zero
        return summed / count
    else:
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        :param model: the model, which takes a signal and a batch of timesteps as input. :param x:
        the [N x C x ...] tensor at time t. :param t: a 1-D Tensor of timesteps. :param
        clip_denoised: if True, clip the denoised signal into [-1, 1]. :param denoised_fn: if not
        None, a function which applies to the x_start prediction before it is used to sample.
        Applies before clip_denoised. :param model_kwargs: if not None, a dict of extra keyword
        arguments to pass to the model. This can be used for conditioning.
    ...
    def condition_mean(self, cond_fn, p_mean_var, x, t, model_kwargs=None):
        ...
        gradient of a conditional log probability with respect to x.
    ...
    def p_sample(
        ...
        """Sample x_{t-1} from the model at the given timestep. :param model: the model to sample
        from. :param x: the current tensor at x_{t-1}. :param t: the value of t, starting at 0 for
        the first diffusion step. :param clip_denoised: if True, clip the x_start prediction to
        [-1, 1]. :param denoised_fn: if not None, a function which applies to the x_start
        prediction before it is used to sample. :param cond_fn: if not None, this is a gradient
        function that acts similarly to the model. :param model_kwargs: if not None, a dict of
        extra keyword arguments to pass to the model. This can be used for conditioning.
    ...
    def _vb_terms_bpd(
        ...
        """Get a term for the variational lower-bound. The resulting units are bits (rather than
        nats, as one might expect). This allows for comparison to other papers.
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
            noise = maybe_noise_like_with_mask(x_start, model_kwargs.get("mask"))
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            terms["mse"] = mean_flat(
                (target - model_output) ** 2, mask=model_kwargs.get("mask")
            )
            if "vb" in terms:
